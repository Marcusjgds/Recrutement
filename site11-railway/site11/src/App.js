    ...
    <div style={{minHeight:"100vh",paddingTop:70}}>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"40px 40px 80px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:28}}>
          <h2 style={{fontFamily:"'Playfair Display',serif",color:"#f0f4ff",margin:0,fontSize:30}}>Postes disponibles <span style={{color:"#C8A96E",fontSize:20}}>({jobs.length})</span></h2>
...
function JobCard({ job, onApply }) {
  ...
  return (
    <div onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)} style={{background:hover?"rgba(30,35,55,0.9)":"rgba(20,24,40,0.8)",border:`1px solid ${hover?"rgba(200,169,110,0.4)":"rgba(255,255,255,0.06)"}`,borderRadius:16,padding:"28px 30px",transition:"all 0.3s",transform:hover?"translateY(-4px)":"none",boxShadow:hover?"0 20px 40px rgba(0,0,0,0.4)":"none"}}>
      ...
      <p style={{color:"#8899aa",fontSize:14,lineHeight:1.7,margin:"0 0 22px",fontFamily:"'Outfit',sans-serif"}}>{job.description}</p>
...
function ApplyModal({ job, onClose, onSuccess }) {
  ...
  const send = async () => {
    if(!form.nom||!form.prenom||!form.email||!form.motivation){alert("Veuillez remplir les champs obligatoires (Nom, Prénom, Email, Motivation).");return;}
    ...
    <div style={{minHeight:"100vh",background:"#080a14"}}>
      <div style={{background:"rgba(14,17,32,0.98)",borderBottom:"1px solid rgba(200,169,110,0.15)",padding:"22px 40px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        <div style={{display:"flex",gap:10}}>
          <button onClick={goHome} style={{background:"rgba(200,169,110,0.1)",border:"1px solid rgba(200,169,110,0.3)",color:"#C8A96E",padding:"8px 16px",borderRadius:8,cursor:"pointer",fontFamily:"'Outfit',sans-serif",fontSize:13}}>← Voir le site</button>
      ...
      <div style={{background:"rgba(10,12,20,0.8)",borderBottom:"1px solid rgba(255,255,255,0.05)",padding:"0 40px"}}>
        <div style={{display:"flex"}}>
          {[["apps",`📩 Candidatures (${applications.length})`],["jobs","📋 Postes"]].map(([t,l])=>(
            <button key={t} onClick={()=>setTab(t)} style={{background:"transparent",border:"none",borderBottom:tab===t?"2px solid #C8A96E":"2px solid transparent",color:tab===t?"#C8A96E":"#8899aa",padding:"14px 28px",cursor:"pointer",fontSize:14,fontFamily:"'Outfit',sans-serif"}}>{l}</button>
          ))}
        </div>
      </div>
